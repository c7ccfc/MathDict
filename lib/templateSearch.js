...
var con = new mysql({
    ...
    database: process.env.SEARCH_DATABASE_NAME,
    socketPath: `/cloudsql/${process.env.SEARCH_CONNECTION_NAME}`
});
...
const body = (lang, word) => {
    let css = fs.readFileSync('/home/hosting_users/mathdict/apps/mathdict_mdtest/style/bodySearch.css').toString();
    let js = fs.readFileSync('/home/hosting_users/mathdict/apps/mathdict_mdtest/style/bodySearch.js').toString();
    ...
    if(wordB === undefined && wordA === undefined){
        ...
        var definition = (!dataA.definition && !dataB.definition) ? `` : `
            <div class='definition'>
                ...
                <p>${dataA.definition}</p>
                <p>${dataB.definition}</p>
            </div>
        `;
